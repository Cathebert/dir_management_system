...
<template>
    <LayoutAuthenticated>
        ...
        <Head title="Add Traditional Authority(T/A)" />
        <SectionMain>
            <SectionTitleLineWithButton :icon="mdiEarth" title="Add Traditional Authority(T/A)" main>
                <BaseButton :route-name="route('admin.district.show',props.id)" :icon="mdiArrowLeftBoldOutline"
                    label="Back" color="white" rounded-full small />
            </SectionTitleLineWithButton>
            <CardBox form @submit.prevent="form.post(route('admin.district.storeArea'))">

                <FormField label="T/A Name" :class="{ 'text-red-400': form.errors.name }">
                    <FormControl v-model="form.name" type="text" placeholder="Enter Area" :error="form.errors.name">
